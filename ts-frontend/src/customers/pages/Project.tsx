...
const UpdateProject: React.FC = () => {
  ...
  const projectUpdateSubmitHandler = async (
  ...
  const handleSubInputChange = (idx: number) => (
    ...
    const name = target.name;
    const newTasks = loadedProject.tasks.map((task: Tasks, sidx: number) => {
  ...
  const handleAddTask = () => {
    setLoadedProject({
      ...loadedProject,
      tasks: loadedProject.tasks.concat([
        { title: '', hours: 0, id: Math.random() },
      ]),
    });
  };
  ...
  const handleRemoveTask = (idx: number) => () => {
    setLoadedProject({
      ...loadedProject,
      tasks: loadedProject.tasks.filter(
        (_s: any, sidx: number) => idx !== sidx
      ),
    });
    setHide(false);
  };
        ...
        <div className="text-center">
          ...
          >
            <div className="center edit-title">
              <input
                type="text"
                className="h1 inputTitle"
                name="name"
                value={loadedProject.name}
            ...
            {loadedProject.tasks.map((task: Tasks, idx: number) => (
              <div className="fields" key={task.id}>
                <input
                  type="text"
                  name="title"
                  value={task.title}
                  onChange={handleSubInputChange(idx)}
                />
                <input
                  type="number"
                  name="hours"
                  value={task.hours === null ? 0 : task.hours}
                  onChange={handleSubInputChange(idx)}
                />
                {idx > 0 && (
                  <div
            ...
            <div className="sidebar card">
              <div title="PDF Settings" className="sidebar__settings">
                <div className="pdfSettings" onClick={openPdfSettings}>
                  <svg>
                    <use href="#settings" xlinkHref="#settings" />
                  </svg>
                </div>
                <Button type="button" onClick={showButton} inverse hide={show}>
                  Generate PDF
                </Button>
                {show && loadedUser && (
                  <Button className="fadeIn" type="button" danger>
                    <PDFDownloadLink
                      document={
                        <Invoice result={onGeneratePdf} user={loadedUser} />
                      }
                      fileName="invoice.pdf"
                    >
                      {({ blob, url, loading, error }) =>
                        loading || delay ? 'Loading...' : 'Download PDF'
                      }
                    </PDFDownloadLink>
                  </Button>
                )}
            ...
            <Modal
              ...
              footer={
                <React.Fragment>
                  <div className="button" onClick={cancelPdfSettings}>
                    CANCEL
                  </div>
